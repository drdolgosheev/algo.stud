...
void JournalNetActivity<numLevels>::dumpJournal(std::ostream& out)
{
    ...
    // prehead is placed before the toPrint and after the last element
        ...
        std::ostream& out) const
{
    ...
    if(timeFrom > timeTo)
        throw std::invalid_argument("asd");

    NodeSkipList<NetActivity, TimeStamp, numLevels>* toPrint = _journal.findFirst(timeFrom);

    if(toPrint == nullptr)
        return;

    while(toPrint->key <= timeTo)
    {
        if(toPrint->value.host == hostSuspicious)
        {
            out << toPrint->key << " " << toPrint->value << std::endl;
        }
