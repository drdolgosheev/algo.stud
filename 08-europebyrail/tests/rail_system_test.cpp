...
TEST(RailSystemTest, testRomeWarsaw) {
    ...
    EXPECT_EQ(actual.cities, std::vector<std::string>({"Rome", "Bern", "Sarajevo", "Budapest", "Bucharest", "Warsaw"}));
}
...
TEST(RailSystemTest, testWarsawParis) {
    ...
    EXPECT_EQ(actual.cities, std::vector<std::string>({"Warsaw", "Bucharest", "Budapest", "Sarajevo", "Bern", "Paris"}));
}
...
TEST(RailSystemTest, testLisbonAthens) {
    ...
    EXPECT_EQ(actual.cities, std::vector<std::string>({"Lisbon", "Madrid", "Bern", "Sarajevo", "Skopja", "Tirane", "Athens"}));
}
...
TEST(RailSystemTest, testNoRoute) {
    ...
    EXPECT_EQ(actual.cities, std::vector<std::string>());
