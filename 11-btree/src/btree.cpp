...
Byte* BaseBTree::search(const Byte* k)
{
    ...
    PageWrapper cur(this);
    cur.readPage(_rootPageNum);
    while(true)
    {
        UShort num = 0;
        while (num <= cur.getKeysNum() && k > cur.getKey(num))
        {
            num++;
        }
        if (num <= cur.getKeysNum() && k == cur.getKey(num))
            return cur.getData();
        else if (cur.isLeaf())
            return nullptr;
        else
            cur.readPage(cur.getCursor(num));
    }
}
...
void BaseBTree::reallocWorkPages()
{
    ...
    {
        // Cormen Introduction to algorithms all info from there
        if (!_rootPage.isFull())
        {
        ...
        {
            PageWrapper s(this); // create new
            s.allocPage(1, false); // leaf = false
            s.setKeyNum(0, true);
            s.setCursor(0, _rootPage.getPageNum());
            ...
            s.setAsRoot(true);
            _rootPage.readPage(getRootPageNum());
            setRootPageNum(s.getPageNum(), true);
        }
    }
...
void BaseBTree::PageWrapper::splitChild(UShort iChild)
{
    ...
    PageWrapper z(_tree);
    PageWrapper y(_tree);
    y.readPage(getCursor(iChild));

    UShort t = _tree->_order;

    z.allocPage((UShort) (t - 1), y.isLeaf());

    copyKeys(z._data + z.getKeyOfs(0), y._data + y.getKeyOfs(t), t - 1);

    if (!y.isLeaf())
    {
        for (int i = 0; i < t; ++i)
            z.setCursor(i, y.getCursor(i+t));
    }

    // this information was got from https://www.geeksforgeeks.org/b-tree-set-1-insert-2/
    //for commit

    y.setKeyNum(t);

    this->setKeyNum((getKeysNum() + 1)); // extend for 1 more key

    this->copyCursors(_data + getCursorOfs(iChild + 1), _data + getCursorOfs(iChild), getKeysNum() - iChild); // copy children

    this->setCursor((iChild + 1), z.getPageNum()); // x's new child is z

    this->copyKeys(_data + getKeyOfs(iChild + 1), _data + getKeyOfs(iChild), getKeysNum() - iChild - 1);

    this->copyKey(_data + getKeyOfs(iChild), y._data + y.getKeyOfs(t - 1));
...
void BaseBTree::PageWrapper::insertNonFull(const Byte* k)
{
    ...
    if (isLeaf())
    {
        // need to insert, so need to extend
        this->setKeyNum(getKeysNum() + 1);
        // go left and shifting, while haven't found place
        while (i >= 0 && c->compare(k, getKey(i), UCHAR_MAX))
        {
            copyKey(_data + getKeyOfs(i + 1), _data + getKeyOfs(i));
            i--;
        }
        // insert key at found position
        copyKey(_data + getKeyOfs(i + 1), k);
        // writing changes to the disk
        this->writePage();
    }
    else // this node is not leaf
    {
        // find the child, where we are inserting with the comparator
        while (i >= 0 && c->compare(k, getKey(i), UCHAR_MAX))
            i--;
        ++i;

        PageWrapper child(_tree); // new child
        child.readPage(getCursor(i));
        ...
        if (child.isFull())
        {
            ...
            // if k > key
            if (!c->compare(k, getKey(i), UCHAR_MAX))
                i++;
            child.readPage(getCursor(i));
        }
